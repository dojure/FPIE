...
def process_command_args(arguments):
    ...
    for args in arguments:
        ...
        if args.startswith("kernel_size"):
            kernel_size = int(args.split("=")[1])

        if args.startswith("depth"):
            depth = int(args.split("=")[1])

        if args.startswith("blocks"):
            blocks = int(args.split("=")[1])

        if args.startswith("parametric"):
            parametric = True

        if args.startswith("s_conv"):
            s_conv = True

        if args.startswith("convdeconv"):
            convdeconv = True
    ...
    return phone, batch_size, train_size, learning_rate, num_train_iters, \
           ...
           dped_dir, vgg_dir, eval_step, run, kernel_size, depth, blocks, parametric, s_conv, convdeconv
...
def process_test_model_args(arguments):
    ...
    return phone, dped_dir, test_subset, iteration, resolution, use_gpu, run, kernel_size, depth, blocks, parametric, s_conv, convdeconv
